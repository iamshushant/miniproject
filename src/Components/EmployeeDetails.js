...
// dynamic graph data
    useEffect(() => {
      ...
      if (data2 && data2.length > 0) {
        const labels = data2.filter((d) => d.uid !== '').map((d) => d.updatedAt.slice(11, 19));
        const labelforJawaan = data2.filter((d) => d.uid !== '').map((d) => d.updatedAt.slice(11, 19));

        // const jawaan = data2.find(d => d.uid === selectedOption);

        const hrData = data2.filter((d) => d.uid !== '').map((d) => d.heartRate);
        const hrvData = data2.filter((d) => d.uid !== '').map((d) => d.heartRateVariability);
        const rrData = data2.filter((d) => d.uid !== '').map((d) => d.respiratoryRate);
        const spo2Data = data2.filter((d) => d.uid !== '').map((d) => d.spo2);
        const tempData = data2.filter((d) => d.uid !== '').map((d) => d.bodyTemperature);
              ...
              {view && 
                <ul className='name-lists'>
                  ...
                  {data ? data.filter((emp)=>{
                            ...
                            else if(emp.name.toLowerCase().includes(inpValue.toLowerCase())){
                            return emp;
                            }
                        }).map((emp)=>{
                  return (
                    <li key={emp.id} onClick={() => { setSelectedOption(emp.name); toggleHandler(); healthScoreFunc(emp.name); }}>
                      {emp.name}
                    </li>)
                }): ''}
              </ul>
        ...
        <Box className='inj-data'>
          ...
          {selectedDetails && (
            <Box sx={{my: 3}}>
              ...
              <Box sx={{my: 3}}>
                <input type='button' value='Download Past Records' className='btns' onClick={downloadCSV} />
              </Box>
            </Box>
          ...
          {selectedDetails && (
            <Box sx={{p: 3}}>
              <h1>{ selectedDetails ? selectedDetails.condition[0].toUpperCase()+ selectedDetails.condition.slice(1) : 'Stable' }</h1>
              <Accordion sx={{my: 2}}>
              ...
              <Accordion sx={{my: 2}}>
                ...
                <AccordionDetails>
                  <Typography>
                    Past Records of Hospitilisation {selectedDetails.hospitalization_records}
                  </Typography>
                </AccordionDetails>
              </Accordion>
              <Accordion sx={{my: 2}}>
            ...
            {selectedDetails && (
              ...
              <Box className='box-3-child'>
                <Box sx={{my: 2}}>
                  ...
                  <Typography variant='h5' className='values'>{vitalsData.spo2}<ArrowUpwardIcon sx={{color: '#3db28c', fontSize: '18px'}} /><ArrowDownwardIcon sx={{color: '#fb7979', fontSize: '18px'}} /></Typography>
                </Box>

                <Box sx={{my: 2}}>
                  <Typography>Heart Rate</Typography>
                  <Typography variant='h5' className='values'>{vitalsData.heartRate}<ArrowUpwardIcon sx={{color: '#3db28c', fontSize: '18px'}} /><ArrowDownwardIcon sx={{color: '#fb7979', fontSize: '18px'}} /></Typography>
                </Box>

                <Box sx={{my: 2}}>
                  <Typography>Heart Rate Variablity</Typography>
                  <Typography variant='h5' className='values'>{vitalsData.heartRateVariability}<ArrowUpwardIcon sx={{color: '#3db28c', fontSize: '18px'}} /><ArrowDownwardIcon sx={{color: '#fb7979', fontSize: '18px'}} /></Typography>
                </Box>
              
                <Box sx={{my: 2}}>
                  <Typography>Body Temperature</Typography>
                  <Typography variant='h5' className='values'>{vitalsData.bodyTemperature}<ArrowUpwardIcon sx={{color: '#3db28c', fontSize: '18px'}} /><ArrowDownwardIcon sx={{color: '#fb7979', fontSize: '18px'}} /></Typography>
                </Box>
              
                <Box sx={{my: 2}}>
                  <Typography>Respiratory Rate</Typography>
                  <Typography variant='h5' className='values'>{vitalsData.respiratoryRate}<ArrowUpwardIcon sx={{color: '#3db28c', fontSize: '18px'}} /><ArrowDownwardIcon sx={{color: '#fb7979', fontSize: '18px'}} /></Typography>
                </Box>
            </Box>
            </Box>
            )}
                  ...
                  <Container sx={{ width: '90%', marginTop: '20px' }}>
                    <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                      <Tabs value={value} onChange={handleChange} aria-label="wrapped label tabs example">
                        ...
                        <Tab label="Heart Rate Variality Data" {...a11yProps(1)} />
                    ...
                    <CustomTabPanel value={value} index={1}>
                      <Typography variant='h6' sx={{mb: 2}}>Heart Rate Variality Data</Typography>
